...
public class Erraticos extends Enemies {
	private Animation<TextureRegion> patrol;
	private Animation<TextureRegion> alert; 
	private Animation<TextureRegion> alertDcha; 

	boolean persiguiendo;//Indica si está persiguiendo al jugador
	...
	public Erraticos(float x, float y, Stage s, GameScreen lvl) {
		...
		patrol = loadFullAnimation("enemies/ErraticoFloats.png", 1, 4, 0.5f, true);
		alert = loadFullAnimation("enemies/ErraticoAlert.png", 1, 4, 0.5f, true);
		alertDcha = loadFullAnimation("enemies/ErraticoAlertDcha.png", 1, 4, 0.5f, true);
		velocidadMaxima = 700;
		aceleracion = 700;
		...
		if (this.getEnabled()) {
			if (this.persiguiendo) {
				...
				if (Math.sqrt(Math.pow(this.getX() - lvl.player.getX(), 2)
			        ...
			        setPosition(this.inicioX, this.inicioY);
				}
			}
