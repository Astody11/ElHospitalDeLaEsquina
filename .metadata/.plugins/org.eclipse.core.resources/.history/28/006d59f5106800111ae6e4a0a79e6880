...
public class Rebanados extends Enemies {
	...
	public Rebanados(float x, float y, Stage s, GameScreen lvl) {
		super(x, y, s, lvl);
		patrol = loadFullAnimation("enemies/RebanadoFloats.png", 1, 11, 0.3f, true);
		hit = loadFullAnimation("enemies/RebanadoAlert.png", 1, 10, 0.3f, true);
		patrolIzq = loadFullAnimation("enemies/RebanadoFloatsIzq.png", 1, 11, 0.3f, true);
		hitIzq = loadFullAnimation("enemies/RebanadoAlertIzq.png", 1, 10, 0.3f, true);
	...
	public void act(float delta) {
		...
		if(this.getEnabled() && this.enemyHitBox.overlaps(this.lvl.player.hitBox) && this.lvl.player.invulnerabilityTime<=0) {
			AudioManager.playSound("audio/sounds/punch.mp3");
		}
